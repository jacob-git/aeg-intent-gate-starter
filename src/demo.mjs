...
export async function createDemoQueue() {
  const queue = new Map();
...
export async function approveRecord(gate, record) {
  if (record.decision.outcome !== "requires_approval") {
    return {
      error: `Cannot approve ${record.decision.outcome} intent.`,
      status: 400,
    };
  }
...
export function renderPage(queue) {
